...
void process_newchannel_request (TCPRequestChannel *_channel){
	...
	//tcp
	TCPRequestChannel* data_channel = new TCPRequestChannel (new_channel_name, portno);
...
int main(int argc, char *argv[]){
	...
	int opt;
	while ((opt = getopt(argc, argv,  "n:p:h:w:b:m:f:r:")) != -1) {
		switch (opt) {
			case 'm':
	...
	struct sockaddr_storage clientAddy;
	socklen_t sin_size;

	while (true) {
		sin_size = sizeof(clientAddy);
		int clientSocket = accept (control_channel->getfd(), (struct sockaddr *) &clientAddy, &sin_size);
		if (clientSocket == -1) {
			perror("error");
			continue;
		}
		TCPRequestChannel* clientChan = new TCPRequestChannel(clientSocket);
